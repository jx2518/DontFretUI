...
export class ScrollProvider {
    ...
    private dragVelocityFilter = new MovingAverageFilter<vec2>(VELOCITY_WINDOW_SIZE,
        vec2.zero,
        averageVec2);
   ...
   */
    resetContentOrigin() {
        ...
        this.contentOrigin = new vec2(this.contentPosition.x + originOffset.x,
    ...
    get scrollPercentage() {
      ...
      MathUtils.clamp(-this.contentOrigin.y + this.contentPosition.y,
          0,
   ...
   */
    setContent(content: SceneObjectBoundariesProvider): void {
        ...
        this.contentScrollLimit = new BufferedBoundariesProvider(this.content,
            Rect.create(0, 0, 0, 0));
   ...
   */
    recomputeBoundaries(): void {
        ...
        this.contentScrollLimit.buffer = Rect.create(invertedLimit * scrollAreaSize.x,
   ...
   */
    ...
    private offsetBetween(a: Rect, b: Rect): Rect {
        return Rect.create(a.left - b.left,
            a.right - b.right,
            a.bottom - b.bottom,
            a.top - b.top);
    }
    ...
    get contentOffset(): Rect {
        return this.offsetBetween(this.scrollArea.boundaries,
    ...
    private update() {
      ...
      initialEdgeSelector.y === 0
        ...
        const frictionResults = this.applyFriction(currentPosition,
            currentVelocity,
            this.decelerateTime,
            deltaTime);
        ...
        const elasticityResults = this.applyElasticity(currentPosition,
            currentVelocity,
            this.elasticTime,
            deltaTime);
   ...
   */
    ...
    private applyFriction(position: vec3,
        velocity: vec2,
        decelerateTime: number,
        deltaTime: number): [vec3, vec2] {
        ...
        if (this.scrollView.enableHorizontalScroll && edgeSelector.x === 0) {
            const smoothResults = smoothSlide(position.x,
                velocity.x,
                decelerateTime,
                deltaTime);
        ...
        if (this.scrollView.enableVerticalScroll && edgeSelector.y === 0) {
            const smoothResults = smoothSlide(position.y,
                velocity.y,
                decelerateTime,
                deltaTime);
    ...
    private applyElasticity(position: vec3,
        velocity: vec2,
        elasticTime: number,
        deltaTime: number): [vec3, vec2] {
        const edgeSelector = this.selectEdgesInsideScrollArea('Content');
        ...
        if (this.scrollView.enableHorizontalScroll && edgeSelector.x !== 0) {
            ...
            const smoothResults = smoothDamp(position.x,
                contentLimitX,
                velocity.x,
                elasticTime,
                deltaTime);
        ...
        if (this.scrollView.enableVerticalScroll && edgeSelector.y !== 0) {
            ...
            const smoothResults = smoothDamp(position.y,
                contentLimitY,
                velocity.y,
                elasticTime,
                deltaTime);
   ...
   */
    public convertLocalOffsetToParentOffset(offset: Rect): Rect {
      ...
      this.convertLocalUnitsToParentUnits(topRightCorner);
        ...
        return Rect.create(bottomLeftOffsetWorld.x,
   ...
   */
    public convertLocalUnitsToParentUnits(localUnits: vec2): vec2 {
        ...
        return new vec3(worldUnits.x, worldUnits.y, 0).div(this.config.screenTransform.getTransform().getWorldScale());
    }
}
